...
function setenvironment(commonUiBaseUrl,callbackWhenDone){
...
function dictionaryInit(jsondictionary,commonUiBaseUrl,callbackWhenDone){
    ...
    if($('script[src*="async.min.js"]')[0]) { //async lib loaded
        ...
        if(asyncLibraryLoading ){
            addEventListener('asyncLoaded', function (e) {
                setenvironment(commonUiBaseUrl,callbackWhenDone);
            }, false);
        }else{
            if(window.async===undefined){
                var initEnv=false;
